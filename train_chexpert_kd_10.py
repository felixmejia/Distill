...
# python3 train_chexpert_kd_10.py --data_dir /workspace/WORKS/DATA/CheXpert-v1.0-small     --img_dir /workspace/WORKS/DATA/     --output_dir chexpert_multistate_results_07052025_1     --epochs 1     --batch_size 1     --lr 0.0001     --img_size 224     --teacher_model densenet121     --student_model mobilenet_v2     --classes Atelectasis Cardiomegaly Consolidation Edema 'Pleural Effusion' > Salida_07052025_1.txt

# python3 train_chexpert_kd_10.py --data_dir /workspace/WORKS/DATA/CheXpert-v1.0-small     --img_dir /workspace/WORKS/DATA/     --output_dir chexpert_multistate_results_01052025_2     --epochs 1     --batch_size 1     --lr 0.0001     --img_size 224     --teacher_model densenet121     --student_model mobilenet_v2     --classes Atelectasis Cardiomegaly Consolidation Edema 'Pleural Effusion' > Salida_01052025_2.txt
...
STATE_SUFFIXES = ['_Present__Uncertain', '_Absent_NotMentioned'] # Usados para nombrar columnas
NUM_STATES = len(STATE_SUFFIXES) # Present, Absent, Uncertain, Not Mentioned
NUM_OUTPUT_NEURONS = NUM_PATHOLOGIES * NUM_STATES # 5 * 2 = 10
...
#
    ...
    train_dataset = ChexpertDataset20BitLabels(os.path.join(args.data_dir, 'train_stratified_split_10bit.csv'), args.img_dir, args.classes, STATE_SUFFIXES, transform=train_transform) 
    valid_dataset = ChexpertDataset20BitLabels(os.path.join(args.data_dir, 'valid_10bit.csv'), args.img_dir, args.classes, STATE_SUFFIXES, transform=val_test_transform)
    test_dataset = ChexpertDataset20BitLabels(os.path.join(args.data_dir, 'test_stratified_split_10bit.csv'), args.img_dir, args.classes, STATE_SUFFIXES, transform=val_test_transform)
